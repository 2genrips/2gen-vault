...
function sourceCount(i){return num(i.source_evidence_count)}
function sourceSummary(i){
  const parts=[];
  if(num(i.official_evidence_count))parts.push(`${num(i.official_evidence_count)} official`);
  if(num(i.partner_evidence_count))parts.push(`${num(i.partner_evidence_count)} partner`);
  if(num(i.storefront_evidence_count))parts.push(`${num(i.storefront_evidence_count)} storefront`);
  return parts.join(' • ')||'No automated provider evidence attached';
}
...
function localIncidents(){
  ...
  for(const p of posts){
    ...
    if(!g){g={id:`local-${groups.length+1}-${key}`,key,room:p.room,product:p.product||p.title||'Community signal',retailer:p.retailer||'',region:p.region||'',status:'WATCH',confidence:0,report_count:0,reporter_count:0,confirm_count:0,gone_count:0,helpful_count:0,source_evidence_count:0,official_evidence_count:0,partner_evidence_count:0,storefront_evidence_count:0,source_confidence:0,source_names:[],first_seen_at:p.created_at||p.createdAt,last_seen_at:p.created_at||p.createdAt,latest_post_id:p.id,posts:[]};groups.push(g)}
...
async function sourceTimeline(i){
  if(!live()||!i?.id)return [];
  try{const {data,error}=await cloud().client.from('source_observations').select('id,source_name,source_type,status,available,quantity,price,confidence,observed_at,expires_at,url').eq('incident_id',i.id).order('observed_at',{ascending:false}).limit(30);if(error)throw error;return data||[]}catch{return []}
}
...
function card(i){const status=displayStatus(i),score=effectiveScore(i),matched=matchesWatch(i);return `<button class="sf-card ${statusClass(i)}" data-incident="${esc(i.id)}"><div class="sf-card-top"><span class="sf-status">${status}</span>${matched?'<span class="sf-match">YOUR WATCH</span>':''}<small>${ageLabel(i.last_seen_at)} ago</small></div><h3>${esc(i.product||'Community signal')}</h3><p>${[i.retailer,i.region].filter(Boolean).map(esc).join(' • ')||'Community'}</p><div class="sf-card-stats"><span><b>${score}</b> confidence</span><span><b>${num(i.report_count)}</b> reports</span><span><b>${sourceCount(i)}</b> sources</span></div></button>`}
function listView(){const list=sortedIncidents(),active=list.filter(i=>['LIVE','WATCH'].includes(displayStatus(i))).length,matched=list.filter(matchesWatch).length;return `<div class="sf-scroll"><section class="sf-hero"><div><span class="sf-kicker">SIGNAL FUSION • v21</span><h2>One drop. One War Room.</h2><p>Community reports and supported provider evidence stay separate, then combine into one explainable incident.</p></div><div class="sf-live-count"><b>${active}</b><span>active</span></div></section><div class="sf-statbar"><span><b>${matched}</b> watch matches</span><span><b>${list.filter(i=>displayStatus(i)==='LIVE').length}</b> live</span><span><b>${list.reduce((a,i)=>a+sourceCount(i),0)}</b> source evidence</span></div>${loading?'<div class="sf-loading">Fusing collector + provider intelligence…</div>':list.length?`<section class="sf-list">${list.map(card).join('')}</section>`:'<div class="sf-empty"><b>No fused incidents yet</b><p>Community reports and supported provider observations will collapse into live War Rooms here.</p></div>'}<div class="sf-footnote">Confidence is an evidence score, not a guarantee that inventory remains available.</div></div>`}
function shell(){return `<div class="sf-shell"><header class="sf-top"><div><div class="sf-mark">≋</div><div><strong>Signal Fusion</strong><span>Community + Source Mesh intelligence</span></div></div><button id="sfClose">×</button></header><main id="sfBody">${listView()}</main></div>`}
...
async function openIncident(id){
  activeId=id;const i=incidents.find(x=>String(x.id)===String(id));if(!i){render();return}
  const [posts,sources]=await Promise.all([timeline(i),sourceTimeline(i)]);const status=displayStatus(i),score=effectiveScore(i);const body=$('#sfBody');if(!body)return;
  const sourceNames=arr(i.source_names).filter(Boolean);
  body.innerHTML=`<div class="sf-scroll"><button class="sf-back" id="sfBack">← ALL INCIDENTS</button><section class="sf-war ${status.toLowerCase()}"><div class="sf-war-top"><span class="sf-status">${status}</span><span>${ageLabel(i.last_seen_at)} ago</span></div><h2>${esc(i.product||'Community signal')}</h2><p>${[i.retailer,i.region].filter(Boolean).map(esc).join(' • ')||'Community'}</p><div class="sf-score"><b>${score}</b><span>FUSION CONFIDENCE</span></div></section><div class="sf-metrics"><div><span>Community reports</span><b>${num(i.report_count)}</b></div><div><span>Collectors</span><b>${num(i.reporter_count)}</b></div><div><span>Source evidence</span><b>${sourceCount(i)}</b></div><div><span>Source score</span><b>${num(i.source_confidence)}</b></div></div><section class="sf-panel"><span class="sf-kicker">WHY THIS SCORE</span><p>Community reports, confirmations and gone reports are scored separately from Source Mesh evidence. Official APIs and authorized partner feeds carry more evidence weight than public storefront availability. Retailer check links carry no stock weight.</p></section><section class="sf-panel"><div class="sf-section-head"><div><span class="sf-kicker">SOURCE MESH</span><h3>${sourceCount(i)} active source observation${sourceCount(i)===1?'':'s'}</h3></div></div><p>${esc(sourceSummary(i))}${sourceNames.length?` • ${sourceNames.map(esc).join(', ')}`:''}</p>${sources.length?sources.map(s=>`<div class="sf-report"><span>${esc(String(s.source_type||'source').replace(/_/g,' ').toUpperCase())}</span><div><b>${esc(s.source_name||'Provider')}</b><small>${ageLabel(s.observed_at)} ago • ${esc(s.available===true?'available':s.status||'unknown')} • confidence ${num(s.confidence)}${s.quantity!=null?` • qty ${num(s.quantity)}`:''}</small></div></div>`).join(''):'<div class="sf-empty small">No provider evidence is attached to this incident.</div>'}</section><section class="sf-panel"><div class="sf-section-head"><div><span class="sf-kicker">COMMUNITY TIMELINE</span><h3>${posts.length} report${posts.length===1?'':'s'} fused</h3></div></div>${posts.length?posts.map(p=>`<div class="sf-report"><span>${esc(p.type||'INFO')}</span><div><b>${esc(p.title||p.product||'Collector report')}</b><small>${ageLabel(p.created_at||p.createdAt)} ago${p.body?` • ${esc(p.body)}`:''}</small></div></div>`).join(''):'<div class="sf-empty small">No community timeline rows available yet.</div>'}</section><div class="sf-actions"><button id="sfMesh">⌁ SOURCE MESH</button><button id="sfStock">◎ FIND STOCK</button><button id="sfSignals">⚡ RAW SIGNALS</button><button id="sfShare">↗ SHARE INTEL</button></div><div class="sf-footnote">VaultSignal does not claim retailer stock unless a supported provider or community report supplies that information. Public storefront availability can change before checkout.</div></div>`;bindDetail(i,posts,sources)
}
function bindDetail(i,posts,sources){$('#sfBack')?.addEventListener('click',()=>{activeId='';render()});$('#sfMesh')?.addEventListener('click',()=>{close();window.VaultSignalSourceMesh?.open?.({incidentId:i.id})});$('#sfStock')?.addEventListener('click',()=>{close();document.querySelector('.bottom-nav [data-tab="stock"]')?.click()});$('#sfSignals')?.addEventListener('click',()=>{close();window.VaultSignalNetwork?.open?.(i.room||'for-you')});$('#sfShare')?.addEventListener('click',()=>shareIncident(i,posts,sources))}
function shareIncident(i,posts,sources){const text=`VaultSignal Signal Fusion\n${displayStatus(i)} • ${effectiveScore(i)} confidence\n${i.product||'Community signal'}\n${[i.retailer,i.region].filter(Boolean).join(' • ')}\n${num(i.report_count)} community reports • ${num(i.reporter_count)} collectors\n${sourceCount(i)} Source Mesh observations • ${sourceSummary(i)}\nUpdated ${ageLabel(i.last_seen_at)} ago\n\nEvidence can change quickly; availability is not guaranteed.`;if(navigator.share)navigator.share({title:'VaultSignal Signal Fusion',text}).catch(()=>{});else navigator.clipboard?.writeText(text)}
...
function joinRealtime(){leaveRealtime();if(!live())return;try{realtime=cloud().client.channel('signal-fusion:v21').on('postgres_changes',{event:'*',schema:'public',table:'signal_incidents'},()=>loadIncidents()).subscribe()}catch{}}
