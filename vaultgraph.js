...
function productsView(s){
  ...
  return `<div class="vg-scroll"><div class="vg-page-head"><div><span class="vg-kicker">PRODUCT TRAIL</span><h2>${esc(itemName(p))}</h2></div></div>
    ...
    <section class="vg-panel"><span class="vg-kicker">PROVENANCE NOTE</span><p>If a rip came from this product but VaultSignal cannot prove it automatically, use Missing Link Center in a future cloud build or add a manual confirmation from the rip view.</p></section>
  </div>`;
}
function ripsView(s){
  ...
  const r=rs.find(x=>itemId(x)===activeRip)||rs.slice().sort((a,b)=>String(dateOf(b)).localeCompare(String(dateOf(a)))[0];activeRip=itemId(r);
...
function cardsView(s){
  ...
  return `<div class="vg-scroll"><div class="vg-page-head"><div><span class="vg-kicker">CARD PROVENANCE</span><h2>Best documented cards</h2></div></div>${cs.slice(0,40).map(c=>{const score=provenanceScore(c,s),down=downstream(c,s);return `<button class="vg-cardrow" data-cardjourney="${esc(itemId(c))}"><div><b>${esc(itemName(c))}</b><small>${esc(itemSet(c)||'Set not recorded')} ${itemNumber(c)?`• #${esc(itemNumber(c))}`:''} • ${down.length} downstream event${down.length===1?'':'s'}</small><div class="vg-progress"><i style="width:${score}%"></i></div></div><strong>${score}%</strong></button>`}).join('')}</div>`;
}
