...
New endpoint: `GET /drop-feed?games=Pokemon,One%20Piece&limit=120`. It checks a starter network of public specialty TCG storefront feeds in parallel. This endpoint is for online availability, not local shelf counts.
...
`GET /local-stock?zip=28761&radius=25&query=Prismatic%20Evolutions%20ETB`

The Worker queries every configured local-stock provider in parallel.
...
The provider should return either a JSON array or:
...
Quantity may be `null` when the source only reports availability.
